...
Each |Agent| in that list has a |Policy|, Dynamics, a list of Sensors, and other attributes about its state, such as its current position, velocity, radius, and goal position.

An external process advances the |environment| by calling :code:`env.step`, which triggers each |Agent| to:
...
Therefore, the external process can pass a dict of actions (keyed by the index of the Agent should take that action) as an argument to the env.step command.
...
You can distinguish Agents who should follow the action dict vs. query their own |Policy| by choosing the appropriate |Policy| sub-class.
Agents whose actions come from an external process can be given an ExternalPolicy.
Since the RL algorithm might not be aware of the Env-specific actions (e.g., RL returns discrete action in {0,1,2,3}, which must be converted to a vehicle speed command), the raw_action from the actions dict is sent through action = ExternalPolicy.convert_to_action(raw_action).
While ExternalPolicy.convert_to_action is a dummy pass-through, you can create a sub-class of ExternalPolicy to implement the desired conversion for your RL output.
