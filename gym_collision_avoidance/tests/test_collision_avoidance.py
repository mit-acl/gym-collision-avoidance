...
class TestSum(unittest.TestCase):
    def test_example_script(self):
        ...
        plot_filename = (
            ...
            + "/../experiments/results/example/000_learning_2agents.png"
        )
    ...
    def test_run_cadrl_formations(self):
        ...
        main = (
            gym_collision_avoidance.experiments.src.run_cadrl_formations.main
        )
        # Check that code runs without error
        self.assertTrue(main())
        ...
        # full episode plots
        for i in range(2):
            plot_filename = os.path.dirname(
                os.path.realpath(__file__)
            ) + "/../experiments/results/cadrl_formations/{}_GA3C-CADRL-10_6agents.png".format(
                str(i).zfill(3)
            )
            self.assertTrue(os.path.isfile(plot_filename))
        # animations
        for i in range(2):
            plot_filename = os.path.dirname(
                os.path.realpath(__file__)
            ) + "/../experiments/results/cadrl_formations/animations/{}_GA3C-CADRL-10_6agents.gif".format(
                str(i).zfill(3)
            )
            self.assertTrue(os.path.isfile(plot_filename))
        ...
        for num_agents in [2, 3, 4]:
            for policy in ["GA3C-CADRL-10", "CADRL", "RVO"]:
                for i in range(3):
                    plot_filename = os.path.dirname(
                        os.path.realpath(__file__)
                    ) + "/../experiments/results/full_test_suites/{num_agents}_agents/figs/{tc}_{policy}_{num_agents}agents.png".format(
                        tc=str(i).zfill(3),
                        policy=policy,
