...
class Neural_network:
    def __init__(
        ...
        self.layers_dim = np.append(
            input_dim, np.append(hidden_layers_size, output_dim)
        )
    ...
    def initialize_nn_weights(self):
        ...
        for i in xrange(self.num_hidden_layers + 1):
            ...
            self.W.append(
                self.nn_training_param.w_scale
    ...
    def train_nn(self, dataset, ERM=0, dataset_test=None):
        ...
        X = (X - np.tile(self.avg_vec, (nb_examples, 1))) / np.tile(
        ...
        for i in xrange(param.nb_iter):
            if (
                ERM == 1 or param.sgd_batch_size > nb_examples
            ):  # if full gradient
                ...
                batch_examples = np.random.permutation(np.arange(nb_examples))[
                    :batch_size
                ]
            ...
            for layer in xrange(nb_layers - 1):
                # RelU
                tmp = np.dot(out, self.W[layer]) + np.tile(
                    self.b[layer], (batch_size, 1)
                )
                forward_prop_o[layer] = tmp * (tmp > 0)
                out = forward_prop_o[layer].copy()
            # last layer, softmax
            scores = np.dot(
                forward_prop_o[-2], self.W[nb_layers - 1]
            ) + np.tile(self.b[nb_layers - 1], (batch_size, 1))
            scores = scores - np.outer(
            ...
            p = np.divide(
                expscores, np.tile(expscores.sum(axis=1), (k, 1)).transpose()
            )
            ...
            if (i % np.ceil(param.nb_iter / 100.0)) == 0:
                ...
                print(
                    "Iter %d, time elapsed: %f, Training 0-1 error: %f, log"
                    " loss: %f, step_size_mode: %s,  step size=%f"
                    % (
                        i,
                        time.time() - t_start,
                        z_train,
                ...
                if self.plotting_func != None and self.X_vis != None:
                    ...
                    self.plotting_func(
                        self.X_vis, Y_vis, title_string, if_new_figure
                    )
                if dataset_test != None:
                    ...
                    X_test = (
                        X_test - np.tile(self.avg_vec, (nb_test_ex, 1))
                    ) / np.tile(self.std_vec, (nb_test_ex, 1))
                    z_test, z_logloss_test = self.evaluate_network_loss(
                        X_test, Y_test
                    )
                    print(
                        "Test 0-1 error: %f, test log loss: %f "
                        % (z_test, z_logloss_test)
                    )
                print(" ")
            ...
            for j in xrange(nb_layers - 1, 0, -1):
                ...
                self.dW[j] = (
                    np.dot(
                        forward_prop_o[j - 1].transpose(), backward_prop_xi[j]
                    )
                ...
                backward_prop_xi[j - 1] = numpy.dot(
                    backward_prop_xi[j], self.W[j].transpose()
                )
                backward_prop_xi[j - 1] = backward_prop_xi[j - 1] * (
                    forward_prop_o[j - 1] > 0
                )

            self.dW[0] = (
                np.dot(X[batch_examples, :].transpose(), backward_prop_xi[0])
            ...
            # method 3: sum of gradients
                ...
                for j in xrange(nb_layers):
                    ...
                    self.W[j] -= (
                        c / (np.sqrt(self.sum_dW[j]) + epsilon) * self.dW[j]
                    )
                    self.b[j] -= (
                        c / (np.sqrt(self.sum_db[j]) + epsilon) * self.db[j]
                    )
    ...
    def make_prediction(self, X):
        ...
        for layer in xrange(nb_layers - 1):
            tmp = np.dot(out, self.W[layer]) + np.tile(
                self.b[layer], (nb_examples, 1)
            )
            out = tmp * (tmp > 0)
        scores = np.dot(out, self.W[nb_layers - 1]) + np.tile(
    ...
    def compute_logloss(self, scores, Y):
        ...
        p = np.divide(
            expscores,
            np.tile(expscores.sum(axis=1), (k, 1)).transpose(),
        )
    ...
    def make_prediction_raw(self, X_raw):
        if len(X_raw.shape) > 1:
            ...
            n = 1
        X = (X_raw - np.tile(self.avg_vec, (n, 1))) / np.tile(
            self.std_vec, (n, 1)
        )
...
if __name__ == "__main__":
    ...
    spiral_dataset = pickle.load(
        open(file_dir + "/test_data/spiral_dataset_train.p", "rb")
    )
    spiral_X = spiral_dataset[0]
    spiral_Y = spiral_dataset[1]
    spiral_X_vis = pickle.load(
        open(file_dir + "/test_data/spiral_dataset_vis.p", "rb")
    )
    ...
    neural_network.set_plotting_func(
        generate_spirals.plot_spiral_datasetWrapper, spiral_X_vis
    )
    ...
    scores = neural_network.make_prediction_raw(spiral_X_vis)
    spiral_Y_vis = np.argmax(scores, axis=1)
    generate_spirals.plot_spiral_dataset(
        spiral_X_vis, spiral_Y_vis, "classification"
    )
