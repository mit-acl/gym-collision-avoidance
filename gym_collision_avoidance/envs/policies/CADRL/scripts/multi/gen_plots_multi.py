...
def plot_training_process(file_dir, format_str, num_agents):
    ...
    test_cases = pickle.load(
        open(
            file_dir
            + "/../../pickle_files/multi/results/%d_agents_hard_test_cases.p"
            % num_agents,
            "rb",
        )
    )
    ...
    for iteration in iterations:
        ...
        # filename=None
        NN_value_net_multi = nn_nav.load_NN_navigation_value(
            file_dir, mode, passing_side, filename=filename
        )

        nn_trajs_filename = (
            file_dir
            + "/../../pickle_files/multi/results/%d_agents_hard_nn_trajs_iter_%d.p"
            % (num_agents, iteration)
        )
        for i, test_case in enumerate(test_cases):
            traj, time_to_complete = NN_value_net_multi.generate_traj(
                test_case, figure_name="%_agents_network" % num_agents
            )
            pedData.plot_traj_raw_multi(
                traj, "", figure_name="training_process"
            )
            plt.title("")
            file_name = (
                "%d_agents_nn_iter_%d_case_%d" % (num_agents, iteration, i)
                + format_str
            )
            plt.savefig(save_folder_dir + file_name, bbox_inches="tight")
            print("saved", file_name)
...
def plot_convergence(file_dir, format_str, num_agents):
    ...
    plt.pause(0.0001)
    plt.savefig(
        save_folder_dir + "/convergence" + format_str, bbox_inches="tight"
    )
...
def plot_value_function(file_dir, format_str):
    ...
    dist_2_other = (
        np.sqrt(np.array([rel_pos_x, rel_pos_y])) - self_radius - other_radius
    )
    x = [
        dist_to_goal,
        pref_speed,
    ...
    nn_navigation = nn_nav.load_NN_navigation_value(
        file_dir, mode, passing_side, filename
    )
    nn_navigation.plot_ped_testCase(x, y, " ", "test_case in no_constr")
    plt.subplot(121)
    plt.title("")
    plt.subplot(122)
    plt.title("")
    ...
    nn_navigation.plot_ped_testCase(x, y, " ", "test_case rotate_constr")
    plt.subplot(121)
    plt.title("")
    plt.subplot(122)
    plt.title("")
...
def plot_multi_agent_cases(file_dir, format_str):
    ...
    filename = "twoAgents_policy_iter_%d.p" % iteration
    # filename=None
    value_net = nn_nav.load_NN_navigation_value(
        file_dir, mode, passing_side, filename
    )
    ...
    traj_raw_multi, time_to_complete = NN_navigation_multi.generate_traj(
        test_cases[2], figure_name="method 1", method=1
    )
    # raw_input()
    plt.title("")
    ...
    traj_raw_multi, time_to_complete = NN_navigation_multi.generate_traj(
        test_cases[3], figure_name="method 1", method=1
    )
    # raw_input()
    plt.title("")
    ...
    for i in xrange(2, 10):
        ...
        num_agents = 4
        side_length = 3
        test_case = nn_nav_multi.generate_rand_test_case_multi(
            num_agents,
            side_length,
            np.array([0.5, 1.2]),
            np.array([0.3, 0.5]),
            is_end_near_bnd=True,
        )
        traj_raw_multi, time_to_complete = NN_navigation_multi.generate_traj(
            test_case, figure_name="method 1", method=1
        )
        plt.title("")
...
def plot_static_case(file_dir, format_str):
    ...
    traj_raw_multi, time_to_complete = NN_navigation_multi.generate_traj(
        test_case, figure_name="method 2", method=1
    )
    plt.title("")
...
def plot_non_coop_case(file_dir, format_str):
    ...
    test_case = np.array(
        [[-3.0, 0.0, 3.0, 0.0, 1.0, 0.5], [3.0, 0.0, -3.0, 0.0, 1.0, 0.5]]
    )
...
def generate_trajs_for_comparison_cases(file_dir, format_str):
    ...
    for i, num_agents in enumerate(num_agents_vec):
        ...
        test_cases = gen_results.generate_test_cases(
            num_test_cases,
            num_agents,
            side_length,
            speed_bnds,
        ...
        filename = (
            file_dir
            + "/../../pickle_files/multi/results/%d_agents_test_cases.p"
            % num_agents
        )
    ...
    for num_agents in num_agents_vec:
        ...
        NN_navigation_multi.generate_trajs_for_testcases(
            test_cases, file_dir=file_dir, filename=save_filename
        )
        print("finished generating trajs for %s" % tc_filename)


def generate_trajs_for_training_multinet(
    file_dir, format_str, num_agents, num_test_cases
):
    save_folder_dir = file_dir + "/../../pickle_files/multi/results/figures/"
    # generate test cases
    # np.random.seed(1)
...
def plot_comparison_multi_agent_traj(file_dir, format_str):
    ...
    for i in xrange(len(traj_names)):
        filename = (
            file_dir
            + "/../../pickle_files/multi/results/%d_agents_%s_trajs_raw.p"
            % (num_agents, traj_names[i])
        )
        trajs.append(pickle.load(open(filename, "rb")))

    # find traj for plotting
    extra_time = pickle.load(
        open(
            file_dir
            + "/../../pickle_files/multi/results/%d_agents_extra_time.p"
            % num_agents,
            "rb",
        )
    )
    for i in xrange(len(extra_time[0])):
        if (
            extra_time[0][i] > extra_time[1][i] + 2
            and extra_time[0][i] > extra_time[2][i] + 2
        ):
            # plot trajs
...
def generate_rvo_offset_cases(file_dir, format_str):
    ...
    for num_agents in num_agents_vec:
        ...
        filename = (
            file_dir
            + "/../../pickle_files/multi/results/%d_agents_rvo_offset_test_cases.p"
            % num_agents
        )
        pickle.dump(test_cases, open(filename, "wb"))
        print("saved %s" % filename)
...
def genenerate_intersection_cases(file_dir, format_str):
    ...
    angles_vec = np.squeeze(np.tile(angles_vec, (1, 5)))
    ...
    for angle in angles_vec:
        test_case = np.array(
            [
                [-sl, 0.0, sl, 0.0, 1.0, 0.3],
                [
                    sl * np.cos(angle),
                    sl * np.sin(angle),
                    sl * np.cos(angle + np.pi),
                    sl * np.sin(angle + np.pi),
                    1.0,
                    0.3,
                ],
            ]
    ...
    filename = (
        file_dir
        + "/../../pickle_files/multi/results/intersection_test_cases.p"
    )
    pickle.dump(test_cases, open(filename, "wb"))
    print("saved %s" % filename)
...
def plot_compare_intersection_cases(file_dir, format_str):
    ...
    for i in xrange(len(traj_names)):
        filename = (
            file_dir
            + "/../../pickle_files/multi/results/intersection_%s_trajs_raw.p"
            % (traj_names[i])
        )
        trajs.append(pickle.load(open(filename, "rb")))
    ...
    for i in xrange(num_test_cases):
        ...
        for j in xrange(len(traj_names)):
            ...
            for k in xrange(5):
                total_mean += np.mean(
                    trajs_stats[j][i + num_test_cases * k][0]
                )
    ...
    (legend_line,) = plt.plot(
        angles_vec, stats[:, 1] - stats[:, 0], linewidth=2, color=plt_colors[0]
    )
    legend_lines.append(legend_line)
    (legend_line,) = plt.plot(
        angles_vec, stats[:, 2] - stats[:, 0], linewidth=2, color=plt_colors[1]
    )
    legend_lines.append(legend_line)
    (legend_line,) = plt.plot(
        angles_vec, stats[:, 3] - stats[:, 0], linewidth=2, color=plt_colors[2]
    )
    legend_lines.append(legend_line)
...
def compute_rvo_offset(file_dir, format_str):
    ...
    for num_agents in num_agents_vec:
        filename = (
            file_dir
            + "/../../pickle_files/multi/results/%d_agents_rvo_trajs_offset.p"
            % num_agents
        )
        trajs.append(pickle.load(open(filename, "rb")))
        stats = gen_results.compute_trajs_stats(trajs[-1])
        ...
        for j in xrange(num_cases):
            offset += (
                np.sum(stats[j][0]) - 4.0 * num_agents
            )  # min time of each agent is 4.0
